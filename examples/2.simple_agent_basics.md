...
### Best Practices
- ✅ Write clear, descriptive docstrings (model reads these!)
...
Get predictable, parseable responses using Pydantic schemas with automatic validation.
...
structured_agent = create_agent(
    ...
    system_prompt="You are a helpful assistant that provides weather information in structured format."
)
...
for msg in result['messages']:
    if type(msg).__name__ == "ToolMessage":
        # Parse and validate
        weather_info = WeatherInfo(**json.loads(msg.content))
...
def custom_error_handler(error: Exception) -> str:
    if isinstance(error, StructuredOutputValidationError):
        return "There was an issue with the format. Please try again."
    else:
        return f"Error: {str(error)}. Please provide valid information."
...
multi_schema_agent = create_agent(
    ...
    system_prompt="Provide weather information OR city information based on the question."
)

# Agent automatically selects the appropriate schema
result = multi_schema_agent.invoke(
    {"messages": [HumanMessage(content="Tell me about Beijing as a city")]},
    config=config
)
```

### Key Features
- **Automatic Validation**: ToolStrategy validates output against Pydantic schema
- **Error Handling**: Built-in error recovery (handle_errors=True by default)
- **Custom Handlers**: Define custom error handling logic
- **Multiple Schemas**: Support Union types for flexible output
- **Type Safety**: Full Pydantic validation with type hints

**Use Cases:** API integrations, database operations, data extraction, consistent formatting
...
- ✅ ReAct logic (Reason → Act → Observe)
...
**Use AGENT when:**
- ✅ Need to call external APIs, databases, calculators
- ✅ Need to remember previous conversation
